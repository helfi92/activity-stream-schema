...
const baseKeys = {
  ...
  client_id: Joi.string().optional().description('Description is super easy to add. It supports `markdown` as well.'),
  addon_version: Joi.string().required().description('Another description.'),
  locale: Joi.string().required().description('And another one'),
...
const UserEventPing = Joi.object().keys(Object.assign({}, baseKeys, {
  ...
  action: Joi.string().valid("activity_stream_user_event").required(),
  metadata_source: Joi.string(),
  highlight_type: Joi.string().valid(["bookmarks", "recommendation", "history"]),
...
const UserEventAction = Joi.object().keys({
  ...
  data: Joi.object().keys({
    ...
    source: Joi.string().valid(["TOP_SITES", "TOP_STORIES"]),
    action_position: Joi.number().integer()
  }).required(),
  meta: Joi.object().keys({
    to: Joi.string().valid(MAIN_MESSAGE_TYPE).required(),
    from: Joi.string().valid(CONTENT_MESSAGE_TYPE).required()
  }).required()
});
...
const UndesiredPing = Joi.object().keys(Object.assign({}, baseKeys, {
  source: Joi.string().required(),
  event: Joi.string().required(),
  action: Joi.string().valid("activity_stream_undesired_event").required(),
...
const ImpressionStatsPing = Joi.object().keys(Object.assign({}, baseKeys, {
  source: Joi.string().required(),
  action: Joi.string().valid("activity_stream_impression_stats").required(),
...
const PerfPing = Joi.object().keys(Object.assign({}, baseKeys, {
  source: Joi.string(),
  event: Joi.string().required(),
  action: Joi.string().valid("activity_stream_performance_event").required(),
  value: Joi.number().required()
}));
...
const SessionPing = Joi.object().keys(Object.assign({}, baseKeys, {
  ...
  action: Joi.string().valid("activity_stream_session").required(),
  perf: Joi.object().keys({
    ...
    // doesn't fire
    load_trigger_type: Joi.string().valid(["first_window_opened",
